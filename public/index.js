...
const createRow = (rowData) => {
    ...
    redirectLink.textContent = shortid;
    if (!redirectURL.startsWith('http://') && !redirectURL.startsWith('https://')) {
        redirectLink.href = `https://${redirectURL}`; 
    } else {
...
const showData = async() => {
    ...
    try {
        ...
        if (shortID.length < 1) {
            urlDisplayDOM.innerHTML = '<h5 class ="empty-list">No Data Available...</h5>'
...
submitButton.addEventListener('click', async (e) => {
    ...
    try {
        ...
        await showConsumptions()  //await (to auto update table data)
